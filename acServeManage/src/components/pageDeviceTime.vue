		  ...
		  <el-tabs :tab-position="tabPosition" style="min-height: 400px;background: #FFFFFF;padding: 10px;">
		    <el-tab-pane label="服务期管理">
			  ...
			  <el-table :data="tableInfo" style="width: 100%;margin: 10px 0;" max-height="600">
			    ...
			    <el-table-column prop="name" label="服务期" width="120"></el-table-column>
	...
	export default {
		...
		methods: {
		    ...
		    doDelete() {
				...
				this.$axios.post(this.$localUrl + 'device/del/time', dataForm, {
		    ...
		    getBrandList: function() {
				this.$axios.get(this.$localUrl + 'device/list/time/'+this.currentPage+'/'+this.pageSizeNum).then((response) => {
			...
			editForm: function(){
				...
				this.brandForm.serveCode = sessionStorage.userCode;
				var dataForm = this.brandForm;
				this.$axios.post(this.$localUrl + 'device/edit/time', dataForm, {
					transformRequest: [function(data) {return JSON.stringify(dataForm);}]
				}).then((response) => {
					this.resetForm();
					this.$message({message: response.data.message,type: 'success'});
			...
			submitForm: function(){
				...
				this.brandForm.serveCode = sessionStorage.userCode;
				var dataForm = this.brandForm;
				this.$axios.post(this.$localUrl + 'device/create/time', dataForm, {
					transformRequest: [function(data) {return JSON.stringify(dataForm);}]
				}).then((response) => {
					this.resetForm();
					this.$message({message: response.data.message,type: 'success'});
